...
```bash
cp .env.example .env         # RevenueCat Test Store key; AdMob app id (the real settle_rewarded unit is used by release builds only)
npx expo prebuild --platform android
npx expo run:android         # physical device; RevenueCat + AdMob need a dev build, not Expo Go
```

A dev build always requests Google's test rewarded unit (AdMob policy — the live unit is used only when `__DEV__` is false, i.e. in a release build). **Server-side verification cannot be enabled on the test unit**, so on a dev build every settle correctly comes back `failed` and the tab stays — the app never pretends otherwise. The first-run protocol and pass criteria are in [docs/SPIKE.md](docs/SPIKE.md).
