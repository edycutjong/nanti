...
| `src/state/AppContext.tsx`           | ledger, prefs, live CustomerInfo, ADS balance (server read), derived `debt`, `wouldBlockExport` (decides) + `recordExport` (commits after the PDF exists)                                                   |
